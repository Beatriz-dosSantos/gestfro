...
export default function Register() {
  ...
  const handleRegister = async () => {
    ...
    try {
      const user = await register(name, email, senha, role);

      // buscar nomeCompleto do Firestore
      const docRef = doc(db, 'funcionarios', user.uid);
      const docSnap = await getDoc(docRef);
      const dados = docSnap.data();
      const nomeFirestore = dados?.nomeCompleto || name;

      // atualizar o displayName com o nome do Firestore
    ...
    <View style={styles.container}>
      ...
      <View style={styles.roleSelector}>
        ...
        <TouchableOpacity
          ...
          style={[
            styles.roleButton,
            role === 'funcionario' && styles.selectedRole,
          ]}
        >
